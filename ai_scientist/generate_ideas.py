...
NOVELTH_SYSTEM_MSG = """You are an ambitious AI PhD student who is looking to publish a paper that will contribute significantly to the field.
...
def generate_ideas(
    ...
    with open(osp.join(base_dir, "prompt.json"), "r", encoding="utf-8") as f:
        prompt = json.load(f)
    if enable_diverse_ideas:
        prompt["system"] = (
            prompt["system"]
            + "Actively seek fresh insights, experiment freely with creative analytical techniques, and propose diverse ideas and title."
        )
    ...
    if save_file_name is not None:
        idea_file_name = "_".join([save_file_name, "ideas.json"])
        ...
        for j in range(max_num_iterations):
            try:
                text, msg_history = get_response_from_llm(
                    ...
                    client=client,
                    model=model,
                    system_message=NOVELTH_SYSTEM_MSG.format(
                        num_rounds=max_num_iterations,
...
def main():
    ...
    parser.add_argument(
        "--disable_seed_ideas",
        action="store_true",
        help="Use seed ideas for idea generation.",
    )
    ...
    ideas = generate_ideas(
        ...
        enable_diverse_ideas=args.enable_diverse_ideas,
    )
